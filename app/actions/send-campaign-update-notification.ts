...
/**
 * Resultado de la acción de envío de notificaciones
 */
...
/**
 * Server Action para enviar notificaciones de actualización de campaña
 * 
 * Esta acción verifica que el usuario sea administrador antes de permitir
 * el envío de notificaciones masivas por WhatsApp.
 * 
 * El template usado está definido en la función de envío y es:
 ...
 * @example
 * ```typescript
 * const result = await sendCampaignUpdateNotificationAction(123);
 * if (result.success) {
 *   console.log(`Enviados: ${result.stats?.successful} mensajes`);
 * }
 * ```
 */
export async function sendCampaignUpdateNotificationAction(
  campaignId: number,
  ...
  try {
    ...
    if (session.user.role !== 'admin') {
      console.warn(
        `[Send Campaign Notification] Usuario ${session.user.email} intentó enviar notificaciones sin permisos`
      );
      return {
        success: false,
    ...
    console.log(
      `[Send Campaign Notification] Admin ${session.user.email} enviando notificaciones${modeLabel} para campaña ${campaignId}`
    );

    // Enviar notificaciones (usa el template por defecto definido en la función)
    const stats = await sendCampaignUpdateNotification({
      campaignId,
      testMode,
    ...
    if (stats.total === 0) {
      return {
        success: false,
        message: stats.errors[0]?.error || 'No hay inversores para notificar.',
        stats,
      };
    }
    ...
    if (stats.successful === stats.total) {
      message = `¡Notificaciones enviadas exitosamente! Se notificó a ${stats.successful} inversor${stats.successful !== 1 ? 'es' : ''}.`;
    ...
    console.log(
      `[Send Campaign Notification] Resultado: ${stats.successful}/${stats.total} exitosos para campaña ${campaignId}`
    );
    ...
    console.error(
      '[Send Campaign Notification] Error inesperado:',
      error
    );
